...
if __name__ == "__main__":
    ...
    objects_opt = {}
    start = get_time()
    for frame_id, det in objects_recon_accu.items():
        obj = optimizer.reconstruct_object(det.t_cam_obj, det.accu_pcd)
        # in case reconstruction fails
        if obj.code is None:
            continue
        objects_opt[frame_id] = obj
    end = get_time()
    print("Reconstructed %d objects in the scene, time elapsed: %f seconds" % (len(objects_opt), end - start))
    ...
    for frame_id, obj in objects_opt.items():
        ...
        # np.savetxt("../../P04_Instance-Completion-and-Motion-Estimation-with-Deep-Shape-Priors-for-Autonomous-Driving/dev_code/opt_nps_accu.txt", opt_nps)
